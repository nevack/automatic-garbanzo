...
fun main() {
    ...
    fun parseLine(line: String): Module {
        ...
        return when (op) {
            ...
            '&' -> Module.Conjunction(name, outs)
            else -> error("ERR!")
        }
    }

    fun parse(input: List<String>): Map<String, Module> {
        val modules = input.map { line -> parseLine(line) }.associateBy { it.name }
        for (con in modules.values.filterIsInstance<Module.Conjunction>()) {
            con.inputs = modules.values.filter { it.outs.contains(con.name) }.map { it.name }
        }
        return modules
    }
...
private sealed class Module(
    ...
    class Conjunction(name: String, outs: List<String>) : Module(name, outs) {
        var inputs: List<String> = listOf()
            set(value) {
                memory = HashMap(value.associateWith { Pulse.LOW })
                field = value
            }
